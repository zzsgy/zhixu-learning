...
test("论文网格在宽屏下也保持单列", () => {
  ...
  assert.match(styles, /#papers-view \.paper-grid \{ grid-template-columns:minmax\(0,1fr\);/);
  assert.doesNotMatch(styles, /#papers-view \.paper-grid \{[^}]*repeat\(2,/);
});

function fixture({ fidelity = "complete", error = false, retry = false } = {}) {
  const { document, Event } = parseHTML(`<html><body><div id="paper-grid"><article class="paper-card">
    ...
    <footer><button class="primary-button">中文阅读</button><a href="https://example.com/paper">论文原文</a><a href="https://example.com/pdf">英文 PDF</a>${retry ? '<button class="secondary-button paper-retry-button">重试解析</button>' : ''}<button class="danger-button">删除</button></footer>
...
test("列表和网格共用两列结构，元数据和横向操作均归入正文", () => {
  ...
  assert.equal(content.querySelector(".paper-card-meta .paper-authors").textContent, "作者");
  assert.equal(content.querySelector(".paper-authors").title, "作者");
...
test("论文导入失败时重试解析直接显示，不再藏进更多菜单", () => {
  const f = fixture({ error: true, retry: true });
  const retry = f.card.querySelector(".paper-retry-button");
  arrangePaperCard(f.card, { ...f, readingStatus: "未读" });
  assert.equal(f.card.querySelector(".paper-card-actions > .paper-retry-button"), retry);
  assert.equal(f.card.querySelector(".paper-card-more-panel .paper-retry-button"), null);
  assert.equal(f.card.querySelector(".paper-card-actions").children[1].textContent, "重试解析");
});
